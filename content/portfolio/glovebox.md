---
title: Glovebox
description: MCP server for privacy-preserving AI analysis — metadata and aggregates only.
---
...
This is the reference implementation of the MCP-style privacy boundary described in [Privacy-preserving MCP-style agents for investigative datasets](/blog/stringboard-mcp-privacy-architecture/). The architecture is general-purpose — Stringboard uses the same pattern, but Glovebox is designed to be dropped into any environment where a model needs to assist with sensitive data without seeing it.
